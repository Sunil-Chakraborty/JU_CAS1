...
class Jrnl_pubForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {            
            ...
            'title_pub'     : widgets.Textarea(attrs={'class':'form-control', 'style': 'height: 5em;font-size:18px;font-weight:bold;text-transform:uppercase',
                                               'rows': 3,'cols': 30}),
            ...
            'role_appl'     : widgets.Select(attrs={                              
                              'class':'form-control',
                              'style': 'width:600px;height:3em;font-size:18px;font-weight:bold;text-transform:uppercase',
                              'oninput': 'check_role()'
                               }),
            'jrnl_name'     : widgets.TextInput(attrs={'class':'form-control','style': 'width:600px;height:3em;font-size:18px;font-weight:bold;text-transform:uppercase'}),
            'vl_pg'         : widgets.TextInput(attrs={'class':'form-control','style': 'width:600px;height:3em;font-size:18px;font-weight:bold;text-transform:uppercase'}),
            ...
            'jrnl_oth'      : widgets.NumberInput(attrs={'class':'form-number form-control','style': 'font-size:18px;font-weight:bold;','oninput': 'limit_other()'})
            ...
            if no_auth == 1 :                
                ...
                if role_appl == "O_A":
                    if not cleaned_data['jrnl_oth']:
                        print(no_auth)
                        raise ValidationError(u'Please provide the no of other authors!')
            #return cleaned_data             
...
class JrnlPaper_UGCForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {            
                    ...
                    'jrnl_name'         : widgets.Textarea(attrs={'class':'form-control', 'style': 'height: 4em;text-transform:uppercase',
                                                        'rows': 3,'cols': 40}),
                    'paper_title'       : widgets.Textarea(attrs={'class':'form-control', 'style': 'height: 4em;text-transform:uppercase'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['yr_review'].widget.attrs['max'] = 2022 # check current year
    ...
    class Meta:
        ...
        widgets = {
            ...
            'prj_amt'      : widgets.NumberInput(attrs={
                                'class':'form-number form-control',                               
                                'oninput': 'limit_input()',
                                'Title' : 'Rs.in lacs only \nfor Arts 2 lacs(min) \nand others 10 lacs(min)'    
                                }),
...
class Resch_guideForm(forms.ModelForm):
    ...
    def clean(self):
            ...
            if degree != 'PHD':
                if sts == 'THES':                  
                   raise ValidationError(u'Check the degree please!!1')
...
class Fellow_AwardForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
             ...
             'name_fellow' : widgets.Textarea(attrs={'class':'form-control', 'style': 'height: 4em;text-transform:uppercase',
                              'rows': 3,'cols': 40}),
             'name_body'   : widgets.Textarea(attrs={'class':'form-control', 'style': 'height: 4em;text-transform:uppercase',
                              'rows': 3,'cols': 40}),
             'prj_lvl'     : widgets.Select(attrs={'class':'form-control','style': 'width:600px;height:3em;text-transform:uppercase'}),             
             'prj_url'     : widgets.URLInput(attrs={'class':'form-control','style': 'width:600px;height:3em;'}),
