...
#from account.views import FilePDF
...
def LetterView(request,*args, **kwargs):   
    ...
    if not casfrmsts.final_sts:
        ...
        sts_idx="Please update below all mandatory pages to proceed  \nviz."+sts_idx
    ...
    if request.method == "GET":
        ...
        if form.is_valid():
            ...
            if form.cleaned_data["is_submit"]:               
                ...
                #return FilePDF.as_view()(self.request)
...
def jrnl_paper_add(request, *args, **kwargs):
    ...
    try :
        jrnl_paper = JrnlPaper_UGC.objects.get(id=id_var)        
    except JrnlPaper_UGC.DoesNotExist:        
        jrnl_paper=JrnlPaper_UGC.objects.create(email_id=request.user.id) 
    ...
    if request.method == 'POST':
            ...
            if form1.is_valid():                            
                ...
                messages.success(request,('Record has been added successfully!'))
                return redirect("jrnl-paper-add")
            else :                
                error_string = ' '.join([' '.join(x for x in l) for l in list(form1.errors.values())])
                messages.error(request,error_string)
                if jrnl_paper:
...
def jrnl_paper_add1(request, pk):
...
def resch_cons_add(request, *args, **kwargs):
    ...
    try :
        cons = Resch_cons.objects.get(id=id_var)        
    except Resch_cons.DoesNotExist:        
...
def lecture_paper_add(request, *args, **kwargs):
    ...
    except Lecture_Paper.DoesNotExist:        
...
def lecture_elearn_add(request, *args, **kwargs):
    ...
    except E_Learning.DoesNotExist:        
...
def cas_sts_view(request,*args, **kwargs):
    ...
    if user.is_admin:
        ...
        cas_sts.annotate(t_gen   = Sum('general_no'), \
                         t_acad  = Sum('academy_no') \
               )
