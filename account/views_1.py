...
class AcademicDetail(objectViewMixin,DetailView):
    model = Academic
...
class ViewPDF(View):
	...
	def get(self, request, *args, **kwargs):
            ...
            if account :   
                        ...
                        template = get_template('account/pdf_template.html')
                        html = template.render(context)
                        
                        response = HttpResponse(content_type='application/pdf')
                        
                        applicant=user.username+".pdf"
                        
                        print(applicant)
                        
                        response['Content-Disposition'] = 'attachment; filename= "' + applicant + '"'
                        ...
                        #response['Content-Disposition'] = 'attachment; filename="casapp.pdf"'
                        response['Content-Disposition'] = 'attachment; filename= "' + applicant + '"'
                        # generate PDF using xhtml2pdf
                        pisa_status = pisa.CreatePDF(html, dest=response)
...
def academy_add(request):
    ...
    if request.method == 'POST':
            ...
            if form1.is_valid():
                prj = form1.save(commit=False)
                prj.email_id = request.user.id
                prj.save()
                messages.success(request,('Record has been added successfully'))
                return redirect("account:academy", user_id=prj.email_id)
            else:                
...
def prests_add(request):
    ...
    if request.method == 'POST':            
            ...
            if form3.is_valid():                
                prj = form3.save(commit=False)
                prj.email_id = request.user.id
                prj.save()
                messages.success(request,('Record has been added successfully!'))
                return redirect("account:professional", user_id=prj.email_id)
            else :                    
...
def curpost_add(request, *args, **kwargs):
    ...
    except PresentPost.DoesNotExist:        
...
def api1_view(request, *args, **kwargs):
    ...
    if request.method == "GET":
        if user_id == 0:
            ...
            msg1='Record has been saved successfully!'            
            context = {
                'form': form,
                'pk_var': pk_var,
                'user_var': user_var,
