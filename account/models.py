...
class Academic(models.Model):
    email 					 = models.ForeignKey(Account,null=True, on_delete = models.SET_NULL)
    degree  				 = models.CharField(verbose_name='Degree/Certificate',max_length=200, null=True)
    ...
    def clean(self):
        ...
        try:
            validator(self.doc_link)
        except:            
            raise ValidationError(u'Check the URL - in models.py line 274')
    """    
...
class Research(models.Model):
    ...
    institute	 		     = models.CharField(verbose_name='Name of the Board/University',max_length=200, null=True, blank=True)
...
class PriorPost(models.Model):
    ...
    designation				 = models.CharField(verbose_name='Designation',max_length=400, choices=DSG_CHOICES, null=True, blank=True)
    employer 				 = models.CharField(verbose_name='Name of Employer',max_length=400, null=True, blank=True)
    dt_join             	 = models.DateTimeField(verbose_name='Date of Joining', null=True, blank=True)
    dt_leav             	 = models.DateTimeField(verbose_name='Date of Leaving', null=True, blank=True)
    gross_salary             = models.CharField(verbose_name='Gross Salary', max_length=20,null=True, blank=True)
    ...
    def clean(self):
        super().clean()
        if not (timezone.now() <= self.dt_join <= self.dt_leav):        
            raise ValidationError('Invalid joining and leaving date')
    """
